...
trait SlidingWarmupSnippet extends Sliding {
  def snippet(n: Rep[Int]): Rep[Array[Int]] = {
    def compute(i: Rep[Int]) = 2*i+3
    n sliding { i => compute(i) + compute(i+1) }
  }
}
abstract class SlidingWarmupDriver extends DslDriver[Int,Array[Int]] with SlidingWarmupSnippet

class SlidingWarmup extends IO {
  val under = "Sliding"
...
abstract class StencilDriver extends DslDriver[Array[Double],Array[Double]] with StencilSnippet

object Stencil extends IO {
  val under = "Stencil"

  val slidingWarmup = new SlidingWarmup

  def run() = {
    slidingWarmup.run()

    println("Stencil tests")
    println("Generating first code snippet in /out/"+under+"-1.actual.scala")
    val stencil0 = new StencilDriver with NoSlidingExp
    exec("-1", stencil0.code)

    println("Generating second code snippet in /out/"+under+"-2.actual.scala")
    val stencil1 = new StencilDriver with SlidingExp
    exec("-2", stencil1.code)
  }
}